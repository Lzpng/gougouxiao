...
window.onload = function () {
    ...
    var vue = new Vue({
        ...
        data: { // 数据模型
            ...
            entity: {specificationOption: []}, // 数据封装对象(表单)SpecificationOption:[]数组使用前要初始化
            page: 1, // 当前页码
            pages: 0, // 总页数
        ...
        methods: { // 定义操作方法
            search: function (page) { // 搜索方法
                // 发送异步请求
                axios.get("/specification/findByPage?page=" + page,
            ...
            saveOrUpdate: function () { // 添加或修改
                ...
                // 发送异步请求
                axios.post("/specification/" + url, this.entity)
                    .then(function (response) {
                        // 获取响应数据
            ...
            show: function (entity) { // 数据回显
                axios.get("/specification/findSpecOption?id=" + entity.id).then(function (value) {
                    entity.specificationOption = value.data;
                    // 把entity对象转化成json字符串
                    var jsonStr = JSON.stringify(entity);
                    // 把json字符串转化成一个新的json对象
                    vue.entity = JSON.parse(jsonStr);
                });
            },
            ...
            del: function () { // 删除
                if (this.ids.length > 0) {
                    axios.get("/specification/delete?ids="
            ...
            addTableRows:function () {
                //点击添加的时候会添加一行{}或者可以new Object
                this.entity.specificationOption.push({});
            },
            //删除一行
            deleteTableRows:function (idx) {
                //根据索引来删除一行
                this.entity.specificationOption.splice(idx,1)
            }
        },
