...
window.onload = function () {
    ...
    var vue = new Vue({
        ...
        data: { // 数据模型
            ...
            page: 1, // 当前页码
            pages: 0, // 总页数
            searchEntity: {status: 0}, // 搜索条件数据封装
            ids: [], // 复选框选中的id数组
        ...
        methods: { // 定义操作方法
            search: function (page) { // 搜索方法
                // 发送异步请求
                axios.get("/seller/findByPage?page=" + page,
            ...
            updateStatus: function (sellerId, status) {
                axios.get("/seller/updateStatus?sellerId=" + sellerId + '&status=' + status).then(function (value) {
                    if (value.data) {
                        vue.search(vue.page);
                    } else {
                        alert("操作失败")
            ...
            saveOrUpdate: function () { // 添加或修改
                ...
                // 发送异步请求
                axios.post("/seller/" + url, this.entity)
                    .then(function (response) {
                        // 获取响应数据
